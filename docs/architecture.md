...
A: I built a transaction monitoring system that mirrors how fraud detection works in real financial institutions — a combination of rule-based flags and an ML model working together. The data is the IEEE-CIS Fraud Detection dataset from Kaggle, about 590,000 transactions with a 3.5% fraud rate. I engineered features for transaction velocity, time-of-day patterns, amount anomalies, and card-testing probe detection, then trained an XGBoost classifier with SMOTE to handle the class imbalance. Every flagged transaction gets a SHAP waterfall explanation so an analyst knows exactly which features drove the score. Everything is exposed in a five-tab Streamlit dashboard where you can adjust thresholds, investigate individual transactions, and batch-score uploaded CSVs.

**Q: Why did you combine rules and ML?**
A: Rules and ML catch different types of fraud. Business rules are fast, interpretable, and immediately actionable — a velocity rule that fires when a card makes six transactions in one hour is something a compliance officer can explain to a regulator without touching the model. They also catch well-known patterns that show up consistently regardless of how the underlying transaction data shifts over time. The ML model fills in the gaps: it picks up on subtle combinations of features that no single rule would catch and generalises to novel fraud patterns the rules weren't written for. In production AML systems at banks, both layers run in parallel. The rule engine feeds the analyst queue directly; the ML model ranks by risk score so the highest-priority cases surface first. Running only rules means you miss complex fraud; running only ML means your decisions are harder to audit and explain to regulators.

**Q: How did you handle the class imbalance?**
A: The dataset is 96.5% legitimate transactions, so a naive model that predicts "not fraud" for everything scores 96.5% accuracy while catching zero fraud — which is why I don't report accuracy at all. I used two complementary strategies. SMOTE (Synthetic Minority Oversampling Technique) generates synthetic fraud samples by interpolating between real ones rather than simply duplicating them, giving the model a richer view of the fraud distribution. It only runs on the training split — applying it to the test set would inflate recall artificially. When the training set has fewer than six fraud samples (which can happen with small uploaded CSVs), the code automatically falls back to XGBoost's scale_pos_weight parameter, which tells the model each fraud case counts roughly 28 times more than a legitimate one during training. Evaluation uses precision, recall, F1, and AUC-ROC because those metrics are sensitive to minority-class performance in a way accuracy is not. Recall is prioritised because a missed fraud case (false negative) costs more than a false alarm (false positive).

**Q: What would you do differently with more time?**
A: A few things. First, I'd add a proper feature store or at least persist the engineered features so the 590K-row pipeline doesn't need to re-run when the app restarts. Second, I'd experiment with model calibration — XGBoost probability outputs aren't always well-calibrated, and in a fraud context you want the 0.8 score to actually mean 80% fraud probability. Third, the SHAP explanations currently run on demand; in production you'd pre-compute them for the entire alert queue during an off-peak batch job rather than on first click. Fourth, I'd add a feedback loop: analysts marking cases as true/false positives is labelled data that should feed back into model retraining. Finally, the velocity features use the full historical dataset; in production you'd need a real-time feature store (Redis or Flink) that maintains rolling counts per customer without scanning the full history on each transaction.

**Q: How would this work in production?**
A: The core pipeline would stay largely the same, but several things would need to change. Data ingestion would be a streaming system — Kafka or Kinesis — rather than a batch CSV load, with transactions scored in near-real-time rather than after the fact. The feature engineering, particularly velocity and probe features, would move to a streaming feature store that maintains per-customer state without loading the full history. The model would be versioned and served via a model registry (MLflow or SageMaker), with canary deployments to compare new model versions against the current one before full rollout. The rule thresholds would be configuration in a database rather than sidebar sliders, with an audit trail of every threshold change. SHAP explanations would be stored alongside each alert in the case management system to satisfy explainability requirements under regulations like SR 11-7. Finally, model performance would be monitored continuously for data drift — fraud patterns evolve and a model trained on 2017-2019 data will degrade over time without periodic retraining on fresh labelled data.
